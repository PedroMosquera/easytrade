...
description: >
  Enable, disable, or verify one of OTel Benchmark App's four problem patterns and confirm it generates
...
You enable, disable, or verify one of OTel Benchmark App's four problem patterns via the
...
| `HighCpuUsage` | Broker-service slow + high CPU; K8s adds CPU limit | `kubectl top pod -n otel-benchmark-app \| grep broker` |
...
| filter k8s.namespace.name == array("otel-benchmark-app")
```

Or check via Dynatrace UI: **Problems** → filter by `otel-benchmark-app` namespace.
