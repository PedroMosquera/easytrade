...
name: service-add
description: >
  Checklist for adding a new microservice to OTel Benchmark App: Dockerfile, compose.dev.yaml entry,
  nginx proxy rule, DB migration script, and Helm values. Trigger phrases: "add a new service",
  "create new microservice", "new OTel Benchmark App service", "add service to otel-benchmark-app".
model: sonnet
tools: Read, Write, Edit, Bash, Grep, Glob
---

You are adding a new microservice to OTel Benchmark App. Work through each step below in order —
...
Expose the service port. Default internal port used by other OTel Benchmark App services: `8080`.
...
Add the service to `helm/otel-benchmark-app/values.yaml` following the pattern of existing services. Each service needs: `image`, `replicaCount`, `env`, and `service.port`.
