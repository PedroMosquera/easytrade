...
### Finding OTel Benchmark App Service Entities

You can find entities via the `find_entities_by_name` tool using `otel-benchmark-app` as well as specific service names (see service list in README).
You should find entities like `[eks-playground][otel-benchmark-app] BrokerService`.
...
```dql
in(k8s.namespace.name, array("otel-benchmark-app")) OR contains(dt.entity.application.name, "OTel Benchmark App")
```
...
Query metrics for all OTel Benchmark App services using the namespace filter:
...
timeseries from:"<ISO-timestamp>", to:"<ISO-timestamp>", by:{dt.entity.service}, interval:1m,
  ...
  filter: k8s.namespace.name == "otel-benchmark-app"
...
You can find additional metrics via `fetch metric.series | filter dt.entity.service == "<service-id>" | limit 50` or for containers: `fetch metric.series | filter k8s.namespace.name == "otel-benchmark-app" | filter metric.key == "dt.kubernetes.container.cpu_usage" or metric.key == "dt.kubernetes.container.memory_working_set" | limit 50`.
