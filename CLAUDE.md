...
./gradlew test --tests "com.dynatrace.otel-benchmark-app.SomeTest"  # single test
```
...
Services are deployed on Kubernetes (namespace `otel-benchmark-app`) and monitored by Dynatrace. See `AGENTS.md` for DQL query patterns, metric keys, and problem investigation workflow. Monaco configurations live in `./monaco/`.
