...
## Description
Helm charts used to deploy *otel-benchmark-app* on Kubernetes cluster using Helm 3+.

## Charts
List of helm charts:
- `otel-benchmark-app` - Umbrella Helm chart to deploy multiple instances of `app` chart with different configuration.
- `otel-benchmark-app/charts/app` - Helm chart for single microservice
...
The otel-benchmark-app chart includes the following microservices:
...
```bash
helm upgrade otel-benchmark-app ./helm/otel-benchmark-app -n otel-benchmark-app -f custom-values.yaml
```
...
```bash
helm lint helm/otel-benchmark-app
```
...
helm template otel-benchmark-app ./helm/otel-benchmark-app -f values.yaml

# Debug mode
helm install otel-benchmark-app ./helm/otel-benchmark-app --dry-run --debug
```
