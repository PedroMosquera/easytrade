...
docker build -t otel-benchmark-appdb .
docker run -d --name db otel-benchmark-appdb

# running with default password passed in ENV
docker run -d --env SA_PASSWORD=yourStrong(!)Password --name db otel-benchmark-appdb
```
