...
* Generating otel-benchmark-app data: generate trading data for initializing otel-benchmark-app db over the period of n days back up until now

```bash
# to generate data for already existing users
poetry run generate_otel-benchmark-app_data {days} --users-file={users_file}

# to generate data with new users
poetry run generate_otel-benchmark-app_data {days} --users-count={users_count}
```